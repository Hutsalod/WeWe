package chat.wewe.android.layouthelper.chatroom.list
...
class RoomMemberListAdapter(private var dataSet: List<User>, private val hostname: String, private val context: Context) : RecyclerView.Adapter<RoomMemberListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val userStatusDrawable: Drawable? = VectorDrawableCompat.create(context.resources, chat.wewe.android.widget.R.drawable.ic_user_status_black_24dp, null)?.mutate()
        DrawableHelper.wrapDrawable(userStatusDrawable)
        when (user.status) {
            User.STATUS_ONLINE -> DrawableHelper.tintDrawable(userStatusDrawable, context, chat.wewe.android.widget.R.color.color_user_status_online)
            User.STATUS_BUSY -> DrawableHelper.tintDrawable(userStatusDrawable, context, chat.wewe.android.widget.R.color.color_user_status_busy)
            User.STATUS_AWAY -> DrawableHelper.tintDrawable(userStatusDrawable, context, chat.wewe.android.widget.R.color.color_user_status_away)
            User.STATUS_OFFLINE -> DrawableHelper.tintDrawable(userStatusDrawable, context, chat.wewe.android.widget.R.color.color_user_status_offline)
        }
